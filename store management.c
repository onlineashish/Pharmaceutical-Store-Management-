...
/*declaration of display sub-menu functions*/
void d_code();
void d_name();
void d_quan();
void d_all();
...
void intro()
 {
    gotoxy(40,7);printf("MINI PROJECT");
    gotoxy(26,8);printf("TOPIC: PHARMACEUTICAL STORE MANAGEMENT ");
    gotoxy(26,11);printf("MENTOR : Mrs. KAVYA B.S. ");
    gotoxy(33,14);printf("TEAM MEMBERS");
    gotoxy(21,15);printf("1. ASHISH KUMAR SAH        (1NT16CS188)");
    gotoxy(21,16);printf("2. PUSHPAM KUMAR	        (1NT16CS167)");
    gotoxy(21,17);printf("3. SAHIB RAMNANI	        (1NT16CS171)");
    gotoxy(21,18);printf("4. SATYAM KUMAR	        (1NT16CS173)");
    gotoxy(21,21);printf("PRESS ANY KEY TO PROCEED...");
    getch();
    system("cls");
 }
...
void d_mainmenu()
{
 int i;
 char ch;
 const char *menu[]={"   Calculate Bill","   Add Item","   Edit Stock","   Display Stock ","   Search an Item", "   Delete an Item","   Exit"};
 system("cls");

 window(45,75,20,32);
 gotoxy(52,18);printf("MAIN MENU");
 for (i=0;i<=6;i++){
  gotoxy(50,23+i);printf("%s\n\n\n",menu[i]);}
  curser(7);
 }
...
void d_search()
{
 char ch;
 int i;
    const char *menu[]={"   By Code","   By Name","   By Quantity","   Back to main menu"};
 system("cls");
...
void highlight(int no,int count)
{
 ...
 if(no==7){
  //textbackground(11);
  //textcolor(0);
  gotoxy (50,23);printf("   Calculate Bill ");
  gotoxy (50,24);printf("   Add Item       ");
  gotoxy (50,25);printf("   Edit Stock     ");
  gotoxy (50,26);printf("   Display Stock  ");
  gotoxy (50,27);printf("   Search an Item ");
  gotoxy (50,28);printf("   Delete an Item ");
  gotoxy (50,29);printf("   Exit           ");
  //textcolor(0);
  //textbackground(2);
  switch(count){
   case 1:
...
void bill()
{
  ...
  while(fread(&item,sizeof(item),1,file))
   if((strcmp(item.code,x)==0)){
        {
        if(q>item.quantity){
            gotoxy(25,22); printf("out of stock");
            goto AGAIN;
        }
    total=item.rate*q;
    gotoxy(25,22);printf("remaining is %d",item.quantity-q);
   ...
   if((strcmp(item.code,x)!=0)){
    gotoxy(25,22);printf("item not found");
   }
    }
 }
 if(gtotal!=0){
...
void d_name(){
  ...
  rewind(file);
  i=26;
  gotoxy(16,20);;printf("enter item name: ");
  gets(a);

  fflush(file);
  while(fread(&item,sizeof(item),1,file)){
  if(strcmp(item.name,a)==0){
   display(&item,i,j);
   i++;
   j++;
   if ((j%20)==0){
...
void del()
{
 ...
 if(flag==0){
  ...
  file2=fopen("record.txt","r+b");
  while (fread(&item,sizeof(item),1,file2)){

   if(strcmp(item.code,x)!=0){
   /* strcpy(updated[i].code,item.code);
    updated[i].rate=item.rate;
    updated[i].quantity=item.quantity;
    strcpy(updated[i++].name,item.name);
   */
   updated[i]=item;
   }
  }
  fclose(file2);
   file2=fopen("record.txt","w+b");
for(j=0;j<i;j++){
    fwrite(&updated[j],sizeof(item),1,file2);
}
